...
function run() {
  ...
  const tx = db.transaction(() => {
    ...
    // ── articles (bilingual — title_ru/excerpt_ru/content_ru) ──────────────────
    const articles = readJson('articles.json', []);
    const insArticle = db.prepare(`INSERT INTO articles
      (slug, title, excerpt, content, title_ru, excerpt_ru, content_ru, category, cover_emoji, author, published_at, active, sort_order)
      VALUES (@slug, @title, @excerpt, @content, @title_ru, @excerpt_ru, @content_ru, @category, @cover_emoji, @author, @published_at, @active, @sort_order)`);
    ...
    // ── courses (leaner field set — no name_ru/age_group/description_ru/features/popular) ──
