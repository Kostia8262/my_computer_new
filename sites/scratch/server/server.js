...
const CONTENT_SEED = {
  ...
  seo: {
    pages: [
      {
        id: 'main',
        label: 'Головна сторінка (scratch.mycomputer.education)',
        heading: 'Курс Scratch для дітей — My Computer Academy',
        text: 'Курс Scratch для дітей 6–10 років у My Computer Academy: блокове програмування, власні ігри та анімації без жодного рядка коду. Малі групи до 5 осіб, перший урок безкоштовно.',
      },
    ],
  },
  faq: [
    { id:1, question:{ ua:'З якого віку можна починати?',       ru:'С какого возраста можно начинать?' },       answer:{ ua:'Приймаємо дітей від 6 років — ідеальний вік для знайомства з логікою через блокове програмування Scratch.', ru:'Принимаем детей от 6 лет — идеальный возраст для знакомства с логикой через блочное программирование Scratch.' } },
    ...
    { id:6, question:{ ua:'Скільки триває курс?',                ru:'Сколько длится курс?' },                     answer:{ ua:'Повний курс Scratch триває 6 місяців при 1–2 заняттях на тиждень — 6 тем від знайомства з редактором до публікації власної гри.', ru:'Полный курс Scratch длится 6 месяцев при 1–2 занятиях в неделю — 6 тем от знакомства с редактором до публикации собственной игры.' } },
  ...
  try {
    if (!coursesDb.getAll().length) {
      coursesDb.create({
        ...
        {"num":"Модуль 3","title":"Інтерактивність","desc":"Обробка подій, клавіатура та миша. Анімовані діалоги та реакція персонажів на дії гравця.","title_ru":"Интерактивность","desc_ru":"Обработка событий, клавиатура и мышь. Анимированные диалоги и реакция персонажей на действия игрока."},
        {"num":"Модуль 4","title":"Ігрова механіка","desc":"Рахунок очок, таймери, рівні складності. Колізії між спрайтами та ігрові правила.","title_ru":"Игровая механика","desc_ru":"Счёт очков, таймеры, уровни сложности. Коллизии между спрайтами и игровые правила."},
        {"num":"Модуль 5","title":"Звук та ефекти","desc":"Додавання звуків, музики та візуальних ефектів. Фінальне полірування проєкту.","title_ru":"Звук и эффекты","desc_ru":"Добавление звуков, музыки и визуальных эффектов. Финальная полировка проекта."},
        {"num":"Фінал","title":"Власна гра","desc":"Кожен учень презентує свою повноцінну гру, розроблену від ідеї до публікації.","title_ru":"Собственная игра","desc_ru":"Каждый ученик представляет свою полноценную игру, разработанную от идеи до публикации."}
        ],
      });
      console.log('✅  Seeded scratch course (6 modules)');
    }
    ...
    if (!adminsDb.getAll().some(a => a.role === 'teacher')) {
      adminsDb.create('Богдан Коваль',   'teacher', { hourlyRate: 150, lessonDuration: 60, phone: '+380501234567', notes: 'Scratch, блокове програмування' });
      adminsDb.create('Аліна Петренко',  'teacher', { hourlyRate: 130, lessonDuration: 60, phone: '+380671234568', notes: 'Scratch для наймолодших' });
      console.log('✅  Seeded 2 test teachers');
    }
    if (!clientsDb.getAll().some(c => c.scheduleDays && c.scheduleDays.length > 0)) {
      [
        { name: 'Марко Тищенко',    age:  8, course: 'scratch', phone: '+380501001001', status: 'active', teacher: 'Аліна Петренко', lessonType: 'group',      scheduleDays: [{day:'1',time:'15:00'},{day:'4',time:'15:00'}], schedule: 'Пн 15:00, Чт 15:00' },
        { name: 'Діана Коваль',     age:  7, course: 'scratch', phone: '+380502002002', status: 'active', teacher: 'Богдан Коваль',  lessonType: 'group',      scheduleDays: [{day:'2',time:'16:00'},{day:'5',time:'16:00'}], schedule: 'Вт 16:00, Пт 16:00' },
        { name: 'Артем Мороз',      age:  6, course: 'scratch', phone: '+380503003003', status: 'active', teacher: 'Богдан Коваль',  lessonType: 'individual', scheduleDays: [{day:'3',time:'17:00'}],                       schedule: 'Ср 17:00' },
        { name: 'Соня Петрик',      age:  9, course: 'scratch', phone: '+380504004004', status: 'active', teacher: 'Аліна Петренко', lessonType: 'group',      scheduleDays: [{day:'3',time:'14:30'}],                       schedule: 'Ср 14:30' },
        ...
        { name: 'Вікторія Лисенко', age: 10, course: 'scratch', phone: '+380506006006', status: 'active', teacher: 'Богдан Коваль',  lessonType: 'group',      scheduleDays: [{day:'2',time:'18:00'}],                       schedule: 'Вт 18:00' },
...
const SCRATCH_INDEX_TPL = fs.readFileSync(path.join(__dirname, '..', 'index.html'), 'utf8');
const SCRATCH_META_RU = {
  title: 'Курс Scratch для детей — My Computer Academy, обучение программированию онлайн',
  desc:  'Курс Scratch для детей 6–10 лет онлайн. Блочное программирование с нуля: собственные игры и анимации без единой строчки кода. Малые группы до 5 человек. Первый урок бесплатно.',
};
app.get(['/', '/index.html'], (req, res, next) => {
  if (req.query.lang !== 'ru') return next();
  const m = SCRATCH_META_RU;
  const html = SCRATCH_INDEX_TPL
...
app.get('/sitemap.xml', (req, res) => {
  const base  = 'https://scratch.mycomputer.education';
...
app.post('/api/leads', leadsLimiter, (req, res) => {
  ...
  try {
    ...
    const sanitized = {
      ...
      source:     'scratch.mycomputer.education',
    };
    ...
    if (process.env.MAIN_ADMIN_TOKEN) {
      fetch('https://mycomputer.education/api/leads/admin', {
        ...
        body: JSON.stringify({ ...sanitized, course: sanitized.course || 'Scratch (лендинг)', notes: 'Заявка з scratch.mycomputer.education' }),
      })
        .then(r => {
...
app.get('/articles/:slug', (req, res) => {
  ...
  <meta property="og:image" content="https://scratch.mycomputer.education/og-image.png?v=2"/>
...
const PRIMARY_COURSE_ID = 'scratch';
