...
function runCode() { clearTimeout(refreshTimer); refreshPreview(); showToast('▶ Запущено!'); }
...
function loadLesson(idx) {
  ...
  const LEVELS = { uk:{easy:'Легко',medium:'Середньо',hard:'Складно',extra:'Бонус'}, ru:{easy:'Легко',medium:'Средне',hard:'Сложно',extra:'Бонус'} };
  const LEVEL = LEVELS[currentLang] || LEVELS.uk;
...
function activatePythonMode(l) {
  ...
  if (l.terminalOutput)
    document.getElementById('terminal-out').textContent = l.terminalOutput;
}
