...
async function loadArticles() {
  ...
  try {
    ...
    if (window.innerWidth >= 769 && prevBtn && nextBtn && totalPages > 1) {
      if (paginationEl) paginationEl.removeAttribute('hidden');
      if (pgDotsEl) {
...
/* ═══════════════════════════════════════════════════════
   WAVE DIVIDER  — animated 3-layer canvas skyline before Contact
   ═══════════════════════════════════════════════════════ */
(function waveDivider() {
  ...
  function buildLayer(layer) {
    const cols = Math.ceil(cssW / layer.bumpW) + 3;
    const bumps = [];
    for (let i = 0; i < cols; i++) {
      bumps.push({ phase: Math.random() * Math.PI * 2, jitter: 0.75 + Math.random() * 0.5 });
    }
    return bumps;
  }
  ...
  function resize() {
    ...
    layerBumps = LAYERS.map(buildLayer);
    stars = buildStars();
    draw();
  }
  ...
  function drawStars() {
    for (const s of stars) {
      const flicker = reduceMotion ? 0.7 : 0.5 + 0.5 * Math.sin(t * 1.6 + s.phase);
      ctx.globalAlpha = 0.25 + flicker * 0.5;
      ctx.beginPath();
  ...
  function drawLayer(layer, bumps) {
    const baseY = cssH;
    const topY  = cssH * (1 - layer.baseRatio);
    const amp   = cssH * layer.ampRatio;
    ...
    for (let i = 0; i < bumps.length; i++) {
      const b  = bumps[i];
      const cx = i * layer.bumpW + r + layer.offset;
      const wobble = reduceMotion ? 0 : Math.sin(t * layer.speed + b.phase) * b.jitter;
      const h = topY - amp * (0.5 + 0.5 * wobble);
      ctx.moveTo(cx - r, baseY);
      ctx.lineTo(cx - r, h);
      ctx.arc(cx, h, r, Math.PI, 0, false);
  ...
  function draw() {
    ...
    drawStars();
    layerBumps.forEach((bumps, i) => drawLayer(LAYERS[i], bumps));
  }
