...
const COURSE_SEED = [
  ...
  { id: 'graphic',   name: 'Графіка та анімація',               emoji: '🎨', age: '6–12 років',  age_group: '6-10',  duration: '3 місяці',  lessonsCount: 24, groupSize: 5, price: 3600, color: '#ec4899', popular: false,
    description: 'Короткий вступний курс цифрового малювання та анімації кадр за кадром у Photoshop — від перших штрихів до власного анімованого мультика.',
    features: [{ua:'✓ Цифровий малюнок у Photoshop', ru:'✓ Цифровой рисунок в Photoshop'},{ua:'✓ Анімація кадр за кадром',            ru:'✓ Анимация кадр за кадром'},{ua:'✓ Власний анімований мультик у фіналі', ru:'✓ Собственный анимированный мультик в финале'}] },
  ...
  { id: 'ui-ux',     name: 'UI/UX дизайн для розробників',      emoji: '📱', age: '12–18 років', age_group: '14-18', duration: '12 місяців', lessonsCount: 96, groupSize: 5, price: 3600, color: '#06b6d4', popular: false,
    description: 'Проєктування інтерфейсів у Figma з нахилом на співпрацю з розробниками: дизайн-токени, Dev Mode, передача макета в код.',
    features: [{ua:'✓ Figma: Auto Layout, компоненти, дизайн-токени', ru:'✓ Figma: Auto Layout, компоненты, дизайн-токены'},{ua:'✓ Handoff макета в код (Dev Mode)', ru:'✓ Хэндофф макета в код (Dev Mode)'},{ua:'✓ Готовий інтерфейс власного застосунку', ru:'✓ Готовый интерфейс собственного приложения'}] },
...
// would otherwise match admin.html first and serve it with the weaker
// "no-cache" (html-file default) instead of "no-store".
...
function requireSuperAdmin(req, res, next) {
  const token = req.headers['x-admin-token'];
  if (!SUPERADMIN_TOKEN || !safeCompare(token, SUPERADMIN_TOKEN)) {
...
app.post('/api/leads', leadsLimiter, (req, res) => {
  ...
  try {
    ...
    const sanitized = {
      ...
      notes:      'Заявка з mycomputer.education',
    };
...
app.get('/api/reviews', (req, res) => {
  const token = req.headers['x-admin-token'];
  const isAdmin = (SUPERADMIN_TOKEN && safeCompare(token, SUPERADMIN_TOKEN)) || !!adminsDb.findByToken(token);
...
// ── COURSE PAGES ──────────────────────────────────────────────────────────────
const DYNAMIC_COURSE_SLUGS = new Set(['scratch', 'python', 'roblox', 'web', 'construct', 'graphic', 'pc', 'blog', 'minecraft', 'ui-ux']);
...
const COURSE_SEO_TITLES = {
  ...
  graphic:   'Курс графіки та анімації для дітей — My Computer Academy',
  pc:        'Базовий курс ПК для дітей — My Computer Academy',
  blog:      'Курс Створення блогу та сайту — My Computer Academy',
  'ui-ux':   'Курс UI/UX дизайну для розробників — My Computer Academy',
};
...
const COURSE_SEO_DESCS = {
  ...
  graphic:   'Курс цифрового малювання та анімації для дітей 6–12 років у Дніпрі. Photoshop, малювання персонажів, анімація кадр за кадром. Онлайн та офлайн у групах до 5 учнів.',
  ...
  'ui-ux':   'Курс UI/UX дизайну для підлітків 12–18 років, які цікавляться IT. Figma, дизайн-токени, передача макета в код. Онлайн та офлайн у малих групах до 5 учнів.',
};
...
const COURSE_SEO_TEXTS = {
  ...
  graphic:   'Курс цифрового малювання та анімації для дітей у Дніпрі від My Computer Academy. Учні 6–12 років роблять перші кроки в Photoshop: малюють персонажів, працюють із шарами та кольором, а потім оживляють свої малюнки за допомогою покадрової анімації. Короткий формат — ідеальний старт перед довшими курсами графіки чи дизайну. У фіналі — власний анімований мультяшний проєкт. Онлайн та офлайн у Дніпрі, мала група до 5 учнів, 24 уроки за 3 місяці. Перший урок безкоштовно.',
  ...
  'ui-ux':   'Курс UI/UX дизайну для підлітків у Дніпрі від My Computer Academy — з нахилом на співпрацю з розробниками. Учні 12–18 років проєктують інтерфейси у Figma: сітки, компоненти, Auto Layout, дизайн-токени. Окрема увага — Dev Mode та передача макета в код, спільна робота дизайнера й розробника. У фіналі — власний клікабельний прототип застосунку. Онлайн та офлайн у Дніпрі, мала група до 5 учнів, 96 занять за 12 місяців. Перший урок безкоштовно.',
};
